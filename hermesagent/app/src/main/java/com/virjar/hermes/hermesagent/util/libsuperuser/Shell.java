...
public class Shell {
    ...
    public static List<String> run(String shell, String[] commands, String[] environment,
        ...
        if (Debug.getSanityChecksEnabledEffective() && Debug.onMainThread()) {
            ...
            // needed here.
            log.error(ShellOnMainThreadException.EXCEPTION_COMMAND);
            throw new ShellOnMainThreadException(ShellOnMainThreadException.EXCEPTION_COMMAND);
        }
        log.info("execute command {}", String.format("[%s%%] START", shellUpper));
        ...
        try {
            ...
            try {
                for (String write : commands) {
                    log.info(String.format("[%s+] %s", shellUpper, write));
                    STDIN.write((write + "\n").getBytes("UTF-8"));
                    STDIN.flush();
                }
                STDIN.write("exit\n".getBytes("UTF-8"));
            ...
            log.info("execute shell exception", e);
            res = null;
        }
        log.info(String.format("[%s%%] END", shell.toUpperCase(Locale.ENGLISH)));
        return res;
    }
    ...
    public static class Builder {
        ...
        private List<Shell.Command> commands = new LinkedList<Command>();
         ...
         * @return Interactive shell
         */
        public Shell.Interactive open(Shell.OnCommandResultListener onCommandResultListener) {
    ...
    public static class Interactive {
        ...
        private Interactive(final Builder builder,
            ...
            if (onCommandResultListener != null) {
                ...
                watchdogTimeout = 60;
                commands.add(0, new Shell.Command(Shell.availableTestCommands, 0, new Shell.OnCommandResultListener() {
        ...
        protected void finalize() throws Throwable {
            if (!closed && Debug.getSanityChecksEnabledEffective()) {
                // waste of resources
                log.error(ShellNotClosedException.EXCEPTION_NOT_CLOSED);
        ...
        private synchronized void handleWatchdog() {
            ...
            if (!isRunning()) {
                ...
                log.info("[{}] SHELL_DIED", shell.toUpperCase(Locale.ENGLISH));
            } else if (watchdogCount++ < watchdogTimeout) {
                return;
            } else {
                exitCode = Shell.OnCommandResultListener.WATCHDOG_EXIT;
                log.info("[{}] WATCHDOG_EXIT", shell.toUpperCase(Locale.ENGLISH));
            }
                ...
                if (command.commands.length > 0) {
                    try {
                        ...
                        for (String write : command.commands) {
                            log.info("[{}] {}", shell.toUpperCase(Locale.ENGLISH), write);
                            STDIN.write((write + "\n").getBytes("UTF-8"));
                        }
        ...
        private synchronized boolean open() {
            log.info("[{}] START", shell.toUpperCase(Locale.ENGLISH));
        ...
        public void close() {
            ...
            if (!_idle && Debug.getSanityChecksEnabledEffective() && Debug.onMainThread()) {
                log.error(ShellOnMainThreadException.EXCEPTION_NOT_IDLE);
            ...
            log.info("[{}] END", shell.toUpperCase(Locale.ENGLISH));
        }
